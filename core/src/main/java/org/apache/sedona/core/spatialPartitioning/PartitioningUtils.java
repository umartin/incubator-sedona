...
public abstract class PartitioningUtils implements Serializable {
    ...
    public static PartitioningUtils getPartitioner(List<Geometry> samples, GridType gridType, Envelope boundaryEnvelope
            , int resolution) {
        ...
        switch (gridType) {
            case EQUALGRID: {
                ...
                try {
                    quadtreePartitioning = new QuadtreePartitioning(new ArrayList<>(), paddedBoundary
                            , resolution, minLevel);
                } catch (Exception e) {
                    e.printStackTrace();
                }
